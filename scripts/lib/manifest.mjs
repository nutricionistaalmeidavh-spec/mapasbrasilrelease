...
function structuralErrors(manifest) {
  ...
  if (typeof manifest.releaseVersion !== 'string' || !RELEASE_VERSION_RE.test(manifest.releaseVersion)) errors.push('releaseVersion must match YYYY.MM.PATCH');
  ...
  manifest.maps.forEach((entry, index) => {
    ...
    if (!Array.isArray(entry.bounds) || entry.bounds.length !== 4 || !entry.bounds.every(Number.isFinite)) errors.push(`${label}.bounds must contain four finite numbers`);
  });
  return errors;
}
...
export async function validateManifest(manifest, catalog) {
  ...
  for (const entry of manifest.maps) {
    ...
    if (entry.name !== stable.name || entry.kind !== stable.kind || entry.minZoom !== stable.minZoom || entry.maxZoom !== stable.maxZoom || !sameArray(entry.bounds, stable.bounds)) errors.push(`stable field mismatch for ${entry.id}`);
    if (Number.isInteger(entry.minZoom) && Number.isInteger(entry.maxZoom) && entry.minZoom > entry.maxZoom) errors.push(`invalid zoom range for ${entry.id}`);
    ...
    if (entry.available === true) {
      ...
      if (typeof entry.asset === 'string') {
        if (entry.asset.includes('/') || entry.asset.includes('\\') || entry.asset.includes('..')) errors.push(`unsafe asset name for ${entry.id}`);
        else if (entry.asset !== expectedAssetName(entry.id)) errors.push(`asset name mismatch for ${entry.id}: expected ${expectedAssetName(entry.id)}`);
      }
      ...
      if (typeof entry.sourceDate !== 'string' || !SOURCE_DATE_RE.test(entry.sourceDate) || Number.isNaN(Date.parse(`${entry.sourceDate}T00:00:00Z`))) errors.push(`invalid sourceDate for ${entry.id}`);
  ...
  for (const id of catalogById.keys()) if (!seen.has(id)) errors.push(`missing catalog id: ${id}`);
